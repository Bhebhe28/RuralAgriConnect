...
const BASE_PROMPT = `You are Mr Mgabhi, an expert AI agricultural advisor for small-scale farmers in KwaZulu-Natal, South Africa. You represent RuralAgriConnect.
...
IDENTITY RULE: Whenever anyone asks who you are, your name, or your identity — in any language — always introduce yourself as Mr Mgabhi from RuralAgriConnect. Never mention Gemini, Google, or any AI provider name.
...
function buildSystemPrompt(language = 'en'): string {
  if (language === 'zu') return BASE_PROMPT + '\n\nCRITICAL LANGUAGE RULE: You MUST respond ONLY in isiZulu for every single reply, no exceptions. Never switch to English regardless of what language the farmer writes in.';
  if (language === 'af') return BASE_PROMPT + '\n\nKRITIESE TAALREËL: Jy MOET altyd en uitsluitlik in Afrikaans antwoord, sonder uitsondering. Moenie oorskakel na Engels nie.';
  if (language === 'st') return BASE_PROMPT + '\n\nMOLAO O BOHLOKOA OA PUO: O TLAMEHA ho araba feela ka Sesotho mesebetsing yohle, ho sa natsoe hore ke puo efe eo molemisi a e sebelisang.';
  return BASE_PROMPT;
}
...
const VISION_MODELS = ['gemini-1.5-flash', 'gemini-2.0-flash', 'gemini-2.0-flash-lite'];

function getModel(apiKey: string, modelName: string, sysPrompt: string) {
  const genAI = new GoogleGenerativeAI(apiKey);
  return genAI.getGenerativeModel({ model: modelName, systemInstruction: sysPrompt });
}

async function tryModels(apiKey: string, sysPrompt: string, fn: (model: ReturnType<typeof getModel>) => Promise<string>): Promise<string> {
  return tryModelsWithList(apiKey, MODELS, sysPrompt, fn);
}

async function tryModelsWithList(apiKey: string, models: string[], sysPrompt: string, fn: (model: ReturnType<typeof getModel>) => Promise<string>): Promise<string> {
  let lastErr: any;
  for (const modelName of models) {
    try {
      const model = getModel(apiKey, modelName, sysPrompt);
...
router.post('/', authenticateFirebase, async (req: AuthRequest, res: Response) => {
  ...
  try {
    const reply = await tryModels(apiKey, sysPrompt, async (model) => {
...
router.post('/scan', authenticateFirebase, upload.single('image'), async (req: AuthRequest, res: Response) => {
  ...
  if (!apiKey || apiKey === 'your_gemini_api_key_here') {
    return res.json({ reply: '🔍 Image analysis requires a Gemini API key. Please configure GEMINI_API_KEY in Firebase Functions environment.' });
  }
  ...
  try {
    ...
    const reply = await tryModelsWithList(apiKey, VISION_MODELS, sysPrompt, async (model) => {
