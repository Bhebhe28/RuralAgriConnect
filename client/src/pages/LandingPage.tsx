...
function Nav() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
      scrolled
        ? isDark ? 'bg-night-surface shadow-lg border-b border-night-border' : 'bg-white shadow-md'
        : 'bg-forest/60 backdrop-blur-md border-b border-white/10'
      ...
      <div className="max-w-6xl mx-auto px-6 h-14 flex items-center justify-between">
        ...
        >
          ...
          <div className="leading-tight">
            <span className={`font-serif font-bold text-base block leading-none ${
              scrolled ? (isDark ? 'text-night-primary' : 'text-forest') : 'text-white'
            }`}>
              RurAgriConnect
            </span>
            <span className={`text-xs ${
              scrolled ? (isDark ? 'text-night-muted' : 'text-muted') : 'text-white/70'
            }`}>KwaZulu-Natal</span>
          </div>
        </div>
          ...
          {[['features','Features'],['knowledge','Knowledge Hub'],['gallery','Gallery'],['pricing','Pricing']].map(([id, label]) => (
            <button key={id} onClick={() => scrollTo(id)}
              className={`transition-colors bg-transparent border-0 cursor-pointer font-medium ${
                scrolled
                  ? isDark ? 'text-night-text/70 hover:text-night-primary' : 'text-dark/70 hover:text-forest'
            ...
            <>
              ...
              <button onClick={() => navigate('/dashboard')}
                className={`text-sm font-bold px-5 py-2.5 rounded-xl transition-all cursor-pointer border-2 ${
                  scrolled
                    ? isDark ? 'bg-night-primary text-night-bg border-night-primary hover:bg-mint' : 'bg-forest text-white border-forest hover:bg-forest-mid'
            ...
            <>
              <button onClick={() => navigate('/login')}
                className={`text-sm font-semibold px-4 py-2 rounded-lg transition-colors bg-transparent border-0 cursor-pointer ${
                  scrolled ? (isDark ? 'text-night-text/70 hover:text-night-primary' : 'text-dark/70 hover:text-forest') : 'text-white hover:text-white/80'
                }`}>
                Sign In
              </button>
              <button onClick={() => navigate('/login')}
        ...
        <div className="md:hidden flex items-center gap-2">
          <ThemeToggle />
          <button
            className={`bg-transparent border-0 cursor-pointer text-2xl ${scrolled ? (isDark ? 'text-night-text' : 'text-dark') : 'text-white'}`}
      ...
      {/* Mobile menu */}
      {menuOpen && (
        <div className={`md:hidden border-t px-6 py-4 flex flex-col gap-4 text-sm font-medium shadow-lg ${
          isDark ? 'bg-night-surface border-night-border' : 'bg-white border-sand'
        }`}>
          {[['features','Features'],['knowledge','Knowledge Hub'],['gallery','Gallery'],['pricing','Pricing']].map(([id, label]) => (
            <button key={id} onClick={() => scrollTo(id)}
          ...
          <div className={`flex gap-3 pt-2 border-t ${isDark ? 'border-night-border' : 'border-sand'}`}>
            {token ? (
              ...
              <>
                <button onClick={() => navigate('/login')} className={`flex-1 py-2.5 border-2 rounded-xl font-semibold bg-transparent cursor-pointer ${isDark ? 'border-night-border text-night-text hover:border-night-primary' : 'border-sand text-dark hover:border-forest'}`}>Sign In</button>
  ...
  return (
    <section className="relative min-h-screen flex items-center overflow-hidden">
      <div className={`absolute inset-0 ${isDark
        ? 'bg-gradient-to-br from-[#050F05] via-[#091509] to-[#0E200E]'
        : 'bg-gradient-to-br from-forest via-forest-mid to-moss'
      }`} />
      ...
      <div className="relative max-w-6xl mx-auto px-4 md:px-6 pb-12 md:pb-20 grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12 items-center w-full"
        ...
        <div className="text-white text-center lg:text-left">
          <div className="inline-flex items-center gap-2 backdrop-blur-sm border rounded-full px-3 py-1.5 text-xs md:text-sm mb-5"
               style={{
                 backgroundColor: isDark ? 'rgba(165,214,167,0.12)' : 'rgba(255,255,255,0.15)',
                 borderColor: isDark ? 'rgba(165,214,167,0.30)' : 'rgba(255,255,255,0.20)',
                 color: isDark ? '#C8E6C9' : '#ffffff',
               }}>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center lg:justify-start">
            <button onClick={() => navigate('/login')}
              style={{ backgroundColor: isDark ? '#A5D6A7' : '#ffffff', color: isDark ? '#051005' : '#1B4332' }}
              className="font-bold px-6 py-3 rounded-xl transition-colors text-sm border-0 cursor-pointer shadow-lg hover:opacity-90">
...
function Features() {
  const { isDark } = useTheme();
  return (
    <section id="features" className={`py-20 ${isDark ? 'bg-night-bg' : 'bg-cream'}`}>
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {FEATURES.map(f => (
            <div key={f.title} className={`rounded-2xl p-6 shadow-sm hover:-translate-y-1 hover:shadow-md transition-all border ${
              isDark ? 'bg-night-card border-night-border' : 'bg-white border-transparent'
            }`}>
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-2xl mb-4 ${isDark ? 'bg-night-primary/15' : 'bg-moss/10'}`}>
                {f.icon}
              </div>
              <h3 className={`font-semibold text-base mb-2 ${isDark ? 'text-night-text' : 'text-dark'}`}>{f.title}</h3>
              <p className={`text-sm leading-relaxed ${isDark ? 'text-night-muted' : 'text-muted'}`}>{f.desc}</p>
            </div>
          ))}
        </div>
      </div>
...
function Gallery() {
  ...
  return (
    <section id="gallery" className={`py-20 ${isDark ? 'bg-night-surface' : 'bg-white'}`}>
      <div className="max-w-6xl mx-auto px-6">
        <div className="text-center mb-14">
          <h2 className={`font-serif text-3xl md:text-4xl mb-3 ${isDark ? 'text-night-text' : 'text-dark'}`}>
            From the Fields of KwaZulu-Natal
          </h2>
          <p className={`text-lg max-w-xl mx-auto ${isDark ? 'text-night-muted' : 'text-muted'}`}>
            The crops our farmers grow — and the advisories that protect them, online or offline
          </p>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {images.map((img, i) => (
            <div key={img.src} onClick={() => navigate('/login')}
              ...
              <div className="absolute top-3 left-3">
                <span className="bg-forest/80 text-white text-xs font-semibold px-2.5 py-1 rounded-full backdrop-blur-sm">{img.tag}</span>
              </div>
            </div>
          ))}
        </div>
...
function HowItWorks() {
  ...
  return (
    <section className={`py-20 ${isDark ? 'bg-night-bg' : 'bg-white'}`}>
      <div className="max-w-6xl mx-auto px-6">
        <div className="text-center mb-14">
          <h2 className={`font-serif text-3xl md:text-4xl mb-3 ${isDark ? 'text-night-text' : 'text-dark'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          {steps.map((s, i) => (
            <div key={s.n} className="relative text-center">
              {i < steps.length - 1 && (
                <div className={`hidden lg:block absolute top-8 left-[60%] w-full h-0.5 z-0 ${isDark ? 'bg-night-border' : 'bg-sand'}`} />
              )}
              ...
              <span className={`text-xs font-bold tracking-widest ${isDark ? 'text-night-primary' : 'text-moss'}`}>{s.n}</span>
              <h3 className={`font-semibold mt-1 mb-2 ${isDark ? 'text-night-text' : 'text-dark'}`}>{s.title}</h3>
              <p className={`text-sm leading-relaxed ${isDark ? 'text-night-muted' : 'text-muted'}`}>{s.desc}</p>
            </div>
          ))}
        </div>
...
// ── Knowledge Hub ─────────────────────────────────────────────────────────────
const ARTICLES = [
  { img: '/img-tomatoes.jpg',    tag: 'Disease Prevention', title: 'Early Blight & Tomato Disease Guide',      desc: 'Identify early blight, late blight, and bacterial wilt in tomatoes. Includes offline-ready prevention tips for KZN conditions.', link: 'Read Advisory →' },
  { img: '/img-maize-field.jpg', tag: 'Maize Advisory',     title: 'Maize Season Advisory — KZN',              desc: 'Planting schedules, fertiliser recommendations, and fall armyworm alerts for KwaZulu-Natal maize farmers.', link: 'View Advisory →' },
  { img: '/img-planting.webp',   tag: 'Pest Alert',         title: 'Fall Armyworm Outbreak Alert',             desc: 'Active outbreak reported in uMgungundlovu. Treatment thresholds, scouting methods, and approved pesticides.', link: 'See Alert →' },
  { img: '/img-root-crops.jpg',  tag: 'Root Crops',         title: 'Root Crop Disease & Soil Health',          desc: 'Prevent root rot, nematode damage, and nutrient deficiency in potatoes, carrots, and beetroot.', link: 'Read Guide →' },
  { img: '/img-farmers.jpg',     tag: 'Community',          title: 'Farmer-to-Farmer Advisory Network',        desc: 'Connect with agricultural extension officers and fellow farmers across KZN to share field observations.', link: 'Join Network →' },
  { img: '/img-groundnut.jpg',   tag: 'Legumes',            title: 'Groundnut & Legume Crop Care',             desc: 'Rosette virus, leaf spot, and aflatoxin prevention for Bambara groundnut and soybean growers in KZN.', link: 'Read Advisory →' },
];
...
function KnowledgeHub() {
  ...
  const { isDark } = useTheme();
  return (
    <section id="knowledge" className={`py-20 ${isDark ? 'bg-night-surface' : 'bg-cream'}`}>
      <div className="max-w-6xl mx-auto px-6">
        <div className="text-center mb-14">
          <h2 className={`font-serif text-3xl md:text-4xl mb-3 ${isDark ? 'text-night-text' : 'text-dark'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
          {ARTICLES.map(a => (
            <div key={a.title} className={`rounded-2xl overflow-hidden shadow-sm hover:-translate-y-1 hover:shadow-md transition-all border ${
              isDark ? 'bg-night-card border-night-border' : 'bg-white border-transparent'
            }`}>
              ...
              <div className="p-5">
                <h3 className={`font-semibold mb-2 ${isDark ? 'text-night-text' : 'text-dark'}`}>{a.title}</h3>
                <p className={`text-sm leading-relaxed mb-4 ${isDark ? 'text-night-muted' : 'text-muted'}`}>{a.desc}</p>
                <button onClick={() => navigate('/login')}
                  className={`text-sm font-semibold transition-colors bg-transparent border-0 cursor-pointer ${
                    isDark ? 'text-night-primary hover:text-mint' : 'text-moss hover:text-forest'
                  }`}>
                  {a.link}
                </button>
              </div>
...
// ── Testimonials ──────────────────────────────────────────────────────────────
const TESTIMONIALS = [
  { stars: 5, quote: 'I was in a remote field with no signal when I spotted something wrong with my maize. I opened RurAgriConnect, got the advisory offline, and treated it the same day.', name: 'Sipho Dlamini', role: 'Smallholder Farmer, uMgungundlovu — KZN', avatar: '👨🏾‍🌾' },
  { stars: 5, quote: 'As an extension officer, I publish advisories and they reach farmers instantly — even those in areas with no internet. The offline sync is exactly what rural KZN needed.', name: 'Nomvula Khumalo', role: 'Agricultural Extension Officer, iLembe District', avatar: '👩🏾‍💼' },
  { stars: 5, quote: 'The multilingual support means my farmers can read advisories in isiZulu. That alone has made a huge difference in how quickly they respond to disease alerts.', name: 'Dr. Thabo Nkosi', role: 'Agricultural Scientist, KZN Dept. of Agriculture', avatar: '👨🏾‍🔬' },
];
...
// ── Pricing ───────────────────────────────────────────────────────────────────
const PLANS = [
  { icon: '🌾', name: 'Farmer', price: 'Free', period: '', desc: 'For individual smallholder farmers — always free', features: ['Full offline advisory access','AI crop disease diagnosis','Weather alerts for your region','Disease prevention tips','isiZulu, Afrikaans & Sesotho support','Push alerts when reconnected'], cta: 'Get Started Free', highlight: false },
  { icon: '👩🏾‍💼', name: 'Extension Officer', price: 'Free', period: '', desc: 'For agricultural officers publishing advisories', features: ['Everything in Farmer plan','Publish crop advisories','Manage farmer accounts','Send region-wide alerts','View farmer activity','Priority support'], cta: 'Register as Officer', highlight: true },
  { icon: '🏛️', name: 'Department / NGO', price: 'Custom', period: '', desc: 'For government departments and agricultural NGOs', features: ['Unlimited officers & farmers','Custom region configuration','Bulk SMS alert integration','Analytics & reporting','White-label option','Dedicated onboarding'], cta: 'Contact Us', highlight: false },
];
...
function Pricing() {
  ...
  const { isDark } = useTheme();
  return (
    <section id="pricing" className={`py-20 ${isDark ? 'bg-night-bg' : 'bg-white'}`}>
      <div className="max-w-6xl mx-auto px-6">
        <div className="text-center mb-14">
          <h2 className={`font-serif text-3xl md:text-4xl mb-3 ${isDark ? 'text-night-text' : 'text-dark'}`}>
            Simple, Transparent Pricing
          </h2>
          <p className={`text-lg ${isDark ? 'text-night-muted' : 'text-muted'}`}>Free for individual farmers — always</p>
        </div>
          ...
          {PLANS.map(p => (
            <div key={p.name} className={`rounded-2xl p-7 relative ${
              p.highlight
                ...
                : isDark ? 'bg-night-card border-2 border-night-border' : 'bg-white border-2 border-sand'
            }`}>
              {p.highlight && (
              ...
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-2xl mb-4 ${p.highlight ? 'bg-white/20' : isDark ? 'bg-night-primary/15' : 'bg-moss/10'}`}>
                {p.icon}
              </div>
              <h3 className={`font-serif text-xl mb-1 ${p.highlight ? 'text-white' : isDark ? 'text-night-text' : 'text-dark'}`}>{p.name}</h3>
              <div className="flex items-baseline gap-1 mb-2">
                <span className={`text-3xl font-bold ${p.highlight ? 'text-white' : isDark ? 'text-night-text' : 'text-dark'}`}>{p.price}</span>
                <span className={`text-sm ${p.highlight ? 'text-white/60' : isDark ? 'text-night-muted' : 'text-muted'}`}>{p.period}</span>
              </div>
              <p className={`text-sm mb-6 ${p.highlight ? 'text-white/70' : isDark ? 'text-night-muted' : 'text-muted'}`}>{p.desc}</p>
              <ul className="space-y-2.5 mb-7">
                {p.features.map(f => (
                  <li key={f} className={`flex items-center gap-2.5 text-sm ${p.highlight ? 'text-white/90' : isDark ? 'text-night-text/80' : 'text-dark/80'}`}>
                    <span className={`text-base ${p.highlight ? 'text-mint' : isDark ? 'text-night-primary' : 'text-moss'}`}>✓</span>
                    {f}
                  </li>
                ))}
              </ul>
              <button
                ...
                className={`w-full py-3 rounded-xl font-semibold text-sm transition-all cursor-pointer border-0 ${
                  p.highlight
                    ...
                    : isDark ? 'bg-night-primary text-night-bg hover:bg-mint' : 'bg-forest text-white hover:bg-forest-mid'
                }`}
              >
                {p.cta} →
              </button>
    ...
    <section className={`py-20 ${isDark ? 'bg-gradient-to-r from-[#091509] to-[#0E200E]' : 'bg-gradient-to-r from-moss to-forest-mid'}`}>
      <div className="max-w-3xl mx-auto px-6 text-center">
        ...
        <div className="flex flex-wrap justify-center gap-4">
          <button onClick={() => navigate('/login')}
            style={{ backgroundColor: isDark ? '#A5D6A7' : '#ffffff', color: isDark ? '#051005' : '#1B4332' }}
            className="font-bold px-8 py-3.5 rounded-xl transition-colors text-sm cursor-pointer border-0 hover:opacity-90">
            Register as a Farmer →
          </button>
          <button onClick={() => navigate('/login')}
            className="border-2 font-semibold px-8 py-3.5 rounded-xl transition-colors text-sm bg-transparent cursor-pointer"
            style={{ borderColor: isDark ? 'rgba(165,214,167,0.5)' : 'rgba(255,255,255,0.5)', color: isDark ? '#A5D6A7' : '#ffffff' }}>
      ...
      <div className="max-w-6xl mx-auto px-6">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10 mb-10">
          <div className="md:col-span-1">
            ...
            <p className="text-sm leading-relaxed mb-4">AI-powered crop disease detection and advisory platform for KwaZulu-Natal farmers.</p>
          ...
          <div>
            ...
            <ul className="space-y-2.5 text-sm">
              {['AI Crop Scanning','Pest Detection','Weather Alerts','Advisory Dashboard','Mobile App'].map(item => (
                <li key={item}>
                  <button onClick={() => navigate('/login')} className="hover:text-white transition-colors bg-transparent border-0 cursor-pointer text-left">{item}</button>
                </li>
              ))}
            </ul>
          </div>
